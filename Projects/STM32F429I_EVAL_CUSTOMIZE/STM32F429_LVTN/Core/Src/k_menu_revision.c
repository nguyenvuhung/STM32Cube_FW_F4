...
static void _OnPaint(BUTTON_Handle hObj) {
  ...
  GUI_DrawBitmap(&bmBKLogo40x20, 0, 0);
}
...
//  FRAMEWIN_SetDefaultFont(GUI_FONT_10_ASCII);
	FRAMEWIN_SetDefaultFont(&GUI_FontTimesNewRoman15);
 ...
 // MULTIPAGE_SetDefaultFont(GUI_FONT_10_ASCII); 
	MULTIPAGE_SetDefaultFont(&GUI_FontTimesNewRoman15);
...
void k_InitMenu(void) 
{
  ...
  ICONVIEW_SetFont(hIcon,&GUI_FontTimesNewRoman17_B);
