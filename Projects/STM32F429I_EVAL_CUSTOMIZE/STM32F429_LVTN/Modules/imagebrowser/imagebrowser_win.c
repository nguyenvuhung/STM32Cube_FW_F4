...
static const GUI_WIDGET_CREATE_INFO _aDialogCreate[] =
{
  ...
  { LISTBOX_CreateIndirect, "List",				ID_IMAGE_LIST,	   570, 10, 220, 320, 0, 0x0, 0 }
};

static const GUI_WIDGET_CREATE_INFO _aSettingsDialogCreate[] =
{
  { FRAMEWIN_CreateIndirect,  SETTINGS/* "Settings"*/,            ID_FRAMEWIN_0,    10, 10, 550, 320, 0, 0x0, 0 },
  { BUTTON_CreateIndirect,    OK /*"OK"*/,                  ID_BUTTON_OK,     350, 50,  100,  50,  0, 0x0, 0 },
  { BUTTON_CreateIndirect,    CANCEL/* "Cancel"*/,              ID_BUTTON_CANCEL, 350, 140, 100,  50,  0, 0x0, 0 },
  { TEXT_CreateIndirect,      SLIDE_TIMER/* "Slide show timer : "*/, ID_TEXT_1,        80,  70, 120, 20,  0, 0x0, 0 },
  { TEXT_CreateIndirect,      SECONDS /*"(seconds)"*/,           ID_TEXT_2,        200, 110, 80,  20,  0, 0x0, 0 }, 
  { SPINBOX_CreateIndirect,   "Spinbox",             ID_SPINBOX_0,     80,  100, 100 ,  40,  0, 0x0, 0 }  
};

static uint8_t _acBuffer[1024*3];
FIL Image_File;
...
static void _cbSettingsDialog(WM_MESSAGE * pMsg)
{
  ...
  case WM_INIT_DIALOG:
   ...
   // BUTTON_SetFont(hItem, GUI_FONT_13B_1);
	  BUTTON_SetFont(hItem, &GUI_FontTimesNewRoman15_B);
 ...
 //   TEXT_SetFont(hItem, GUI_FONT_13B_1);
    TEXT_SetFont(hItem, &GUI_FontTimesNewRoman15_B);
...
/**
  ...
  */
static int _GetData_PNG(void * p, const U8 ** ppData, unsigned NumBytesReq, U32 Off)
{
...
static int _DrawImage(FIL *hFile, uint8_t Enlarge, uint8_t type) 
{
  ...
  if(Enlarge == 0)
  {
    nx = 550000 / Info.XSize;
    ny = 320000 / Info.YSize;
  }
  else
  {
    nx = 800000 / Info.XSize;
...
static void _cbImageWindow(WM_MESSAGE * pMsg)
{
    ...
    if(pImageList->ptr > 0)
    {
      if (pState) {
        if (pState->Pressed == 1)
        {
          ...
          {
            WM_AttachWindowAt(pMsg->hWin, IMAGE_hWin, 10, 10);
            WM_SetSize(pMsg->hWin, 550, 320);
          }
  ...
  case WM_PAINT:
    ...
    if (pImageList->ptr > 0)
    {
		//	hMem=GUI_MEMDEV_Create(20,20,240,120);
		//	GUI_MEMDEV_Select(hMem);
			_DrawImage(&Image_File, IMAGE_Enlarge, IMAGE_Type);
			//GUI_MEMDEV_Select(0);
...
static void _cbDialog(WM_MESSAGE * pMsg)
{
  ...
  case WM_INIT_DIALOG:
    ...
    imFrame = WM_CreateWindowAsChild(r.x0 + 10, r.y0 + 10, r.x1 - 250, r.y1-160, hItem, WM_CF_SHOW, _cbImageWindow, 0);
    
    /* Buttons initialization */
    
    hItem = BUTTON_CreateEx(140, 365, 70, 70, pMsg->hWin, WM_CF_SHOW, 0, ID_PREVIOUS_BUTTON);
		WM_SetCallback(hItem, _cbButton_previous);
    
		hItem = BUTTON_CreateEx(245, 360, 80, 800, pMsg->hWin, WM_CF_SHOW, 0, ID_SLIDE_BUTTON);
    WM_SetCallback(hItem, _cbButton_play);

		hItem = BUTTON_CreateEx(365, 365, 70, 70, pMsg->hWin, WM_CF_SHOW, 0, ID_NEXT_BUTTON);
    WM_SetCallback(hItem, _cbButton_next);
    
		hItem = BUTTON_CreateEx(695, 365, 70, 700, pMsg->hWin, WM_CF_SHOW, 0, ID_ADD_BUTTON);
    WM_SetCallback(hItem, _cbButton_add);
    
		hItem = BUTTON_CreateEx(590, 365, 70, 70, pMsg->hWin, WM_CF_SHOW, 0, ID_OPEN_BUTTON);
    WM_SetCallback(hItem, _cbButton_open);
    
		hItem = BUTTON_CreateEx(475, 370, 60, 60, pMsg->hWin, WM_CF_SHOW, 0, ID_SETTINGS_BUTTON);
    WM_SetCallback(hItem, _cbButton_settings);
    
		hItem = BUTTON_CreateEx(35, 370, 60, 60, pMsg->hWin, WM_CF_SHOW, 0, ID_CLOSE_BUTTON);
 ...
 case WM_PAINT:
    DrawRect3D(25, 350, 520, 90);
    DrawRect3D(580, 350,195, 90);
  ...
  case WM_NOTIFY_PARENT:
    ...
    if (NCode == WM_NOTIFICATION_CHILD_DELETED)
    {
      ...
      {
        if((	strstr(pFileInfo->pRoot, ".jpg")) 
					|| (strstr(pFileInfo->pRoot, ".bmp")) 
					|| (strstr(pFileInfo->pRoot, ".JPG"))
					|| (strstr(pFileInfo->pRoot, ".BMP")))
        {
    ...
    case ID_OPEN_BUTTON: 
      ...
      case WM_NOTIFICATION_RELEASED:
        if((chooser_addfolder == 0) && (chooser_addfile == 0))
        {
          ...
          chooser_addfolder = CHOOSEFILE_Create(pMsg->hWin, 10, 10, 530, 300, apDrives, GUI_COUNTOF(apDrives), 0, ADD_A_FOLDER, 0, pFileInfo);    
 	//				CHOOSEFILE_SetButtonText(pMsg->hWin, CHOOSEFILE_BI_CANCEL, "CANCEL"); 
	//				CHOOSEFILE_SetButtonText(pMsg->hWin, CHOOSEFILE_BI_OK, "OK"); 
	//				CHOOSEFILE_SetButtonText(pMsg->hWin, CHOOSEFILE_BI_UP, "UP");       
				}
        break;
      }
      break;
    ...
    case ID_ADD_BUTTON:  
      ...
      case WM_NOTIFICATION_RELEASED:
        ...
        if((chooser_addfolder == 0) && (chooser_addfile == 0))
        {
          ...
          chooser_addfile = CHOOSEFILE_Create(pMsg->hWin, 10, 10, 530, 300, apDrives, GUI_COUNTOF(apDrives), 0, ADD_AN_IMAGE, 0, pFileInfo);    
	//				CHOOSEFILE_SetButtonText(pMsg->hWin, CHOOSEFILE_BI_CANCEL, "CANCEL"); 
	//				CHOOSEFILE_SetButtonText(pMsg->hWin, CHOOSEFILE_BI_OK, "OK"); 
	//				CHOOSEFILE_SetButtonText(pMsg->hWin, CHOOSEFILE_BI_UP, "UP"); 
				}
